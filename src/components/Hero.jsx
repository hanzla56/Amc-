                ...
                {herodata?.map((item, i) => (
                    <SwiperSlide key={item.id || i}>
                        <div
                            ...
                            style={{ backgroundImage: `linear-gradient(358.93deg, #0D0C0F 0.83%, rgba(13, 12, 15, 0.85) 28.55%, rgba(13, 12, 15, 0) 48.81%, rgba(13, 12, 15, 0.284314) 70.66%, #0D0C0F 103.18%), url(${item?.img.src})` }}>

                            <div className='absolute bottom-10 2xl:bottom-20
                             left-0 mx-auto right-0 z-10 max-w-[1150px] 2xl:max-w-[1550px]'>
                                <Button variant="contained" className='btnadd' style={{background:"#0D0C0F",borderRadius:"50px"}}>
                                {item?.category}
                                </Button> 
                                <h1 className='text-[32px] font-bold mt-2'>{item?.title}</h1>
                                <p className="2xl:text-[16px] text-[14px] my-3">{item?.time}</p>
                                <p  className="2xl:text-[16px] text-[14px] max-w-[550px] my-3">{item?.desc}</p>
